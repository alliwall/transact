...
providerSearch.addEventListener("input", () => {
  ...
  document.querySelectorAll(".provider-item").forEach((t) => {
    t.querySelector(".provider-name").textContent.toLowerCase().includes(e)
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      let l = await fetchExternalApi(
        `https://api.transact.st/control/wallet.php?address=${t}&callback=${n}`
      );
      ...
      if (o && o.address_in) {
        let c = o.address_in;
        o.callback_url;
        let d = `https://payment.transact.st/process-payment.php?address=${c}&amount=${s}&provider=${i}&email=${encodeURIComponent(
            a
          )}&currency=${r}`,
    ...
    `),
          ...
          document
            ...
            .addEventListener("click", function () {
              copyToClipboard(
                `https://payment.transact.st/control/track.php?address=${c}`,
                this
              );
            }),
          document
        ...
        showValidationError(
          "Invalid payment wallet. Please enter a valid wallet address."
        );
    } catch (m) {
      console.error("Error:", m),
        showValidationError(
          `An error occurred while processing your request: ${m.message}`
        );
    } finally {
      stopLoading();
    }
  ...
  navigator.share
    ? navigator
        .share({
          ...
          text: `Use this link to make a payment of ${a} ${t}:`,
          url: e,
        })
        .then(() => showToast("Link shared successfully", "success"))
        .catch((e) => console.log("Error sharing:", e))
    : (copyToClipboard(e, document.getElementById("copy-payment-link")),
      showToast("Link copied to clipboard", "success"));
}
function fixDarkModeStyles() {
  if (
    "dark" === document.body.getAttribute("data-theme") ||
    "dark" === document.body.getAttribute("data-bs-theme")
  ) {
    document
      .querySelectorAll(".copy-link-input, #payment-link")
      .forEach((e) => {
    ...
    document
      .querySelectorAll(".card-header .btn-light, .card-header .btn-sm")
      .forEach((e) => {
        (e.style.backgroundColor = "#334155"),
          (e.style.color = "#f8fafc"),
          (e.style.borderColor = "#475569");
      });
  }
}
function filterProvidersByCurrency(e) {
  ...
  t.forEach((t) => {
    ...
    "ALL" === r || r === e
      ? ((t.style.display = "block"), a || ((s.checked = !0), (a = !0)))
      : ((t.style.display = "none"), (s.checked = !1));
  });
}
...
providerInputs.forEach((e) => {
  ...
  document.getElementById("amount").addEventListener("change", function () {
    let e = parseFloat(this.value),
      ...
      a = minAmounts[t] || 0;
    e < a
      ? (this.setCustomValidity(`Minimum amount for ${t} is ${a}`),
        showToast(`Minimum amount for ${t} is ${a}`, "warning"))
  ...
  document
    ...
    .addEventListener("input", function () {
      ...
      e && !/^0x[a-fA-F0-9]{40}$/.test(e)
        ? (this.classList.add("is-invalid"),
          this.setCustomValidity("Invalid wallet address"))
        : (this.classList.remove("is-invalid"), this.setCustomValidity(""));
    }),
