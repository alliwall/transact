...
async function filterProvidersByCurrency(currency) {
    ...
    // Obter restrições de provedores da URL, se presentes
    ...
    if (encryptedProviders) {
        try {
            // Descriptografar lista de provedores
            const providersString = await decryptData(encryptedProviders);
            if (providersString) {
                allowedProviders = providersString.split(',');
            }
        } catch (error) {
            console.error("Erro ao descriptografar provedores durante filtro:", error);
        }
    }
    
    // Verificar se os provedores têm o atributo data-currency
    providerItems.forEach(item => {
        const dataCurrency = item.getAttribute('data-currency');
        if (dataCurrency) {
            // Verificar se o data-currency corresponde à moeda selecionada
            if (dataCurrency.toLowerCase() === currency.toLowerCase() || dataCurrency.toLowerCase() === 'all') {
                // Verificar se o provedor está na lista permitida (se existir)
                const providerInput = item.querySelector('input[name="provider"]');
                if (providerInput) {
                    const providerValue = providerInput.value;
                    const isAllowed = !allowedProviders || allowedProviders.includes(providerValue);
                    ...
                    if (isAllowed) {
                        ...
                        // Selecionar o primeiro provedor visível
                        if (!foundChecked) {
                            providerInput.checked = true;
                            foundChecked = true;
                            
                            // Atualizar o valor mínimo
                            const minAmount = minAmounts[providerValue] || 0;
                            const amountInput = document.getElementById("amount");
                            if (amountInput) {
                                amountInput.setAttribute("min", minAmount);
            ...
            // Sem atributo data-currency, verificar data-supported-currency
            const providerInput = item.querySelector('input[name="provider"]');
            if (providerInput) {
                const supportedCurrency = providerInput.getAttribute("data-supported-currency");
                const providerValue = providerInput.value;
                ...
                // Mostrar o provedor se ele suportar a moeda E estiver na lista permitida
                if ((supportedCurrency === "ALL" || supportedCurrency === currency) && isAllowed) {
                    ...
                    // Não corresponde à moeda ou não está permitido
                    item.style.display = "none";
                    providerInput.checked = false;
                }
            }
    ...
    // Se nenhum provedor foi selecionado, tentar encontrar qualquer provedor visível
    if (!foundChecked) {
        ...
        if (firstVisibleProvider) {
            ...
            firstVisibleProvider.disabled = false; // Garantir que está ativado
            ...
            console.warn("Nenhum provedor disponível para a moeda " + currency);
        }
    }
    
    // Verificação final para garantir que todos os provedores visíveis estão ativados
...
document.addEventListener("DOMContentLoaded", function () {
    // Configurar o listener de alteração de moeda
    const currencySelect = document.getElementById("currency");
    if (currencySelect) {
    ...
    // Definir atributos de moeda suportada para os provedores se ainda não estiverem definidos
    document.querySelectorAll('input[name="provider"]').forEach((input) => {
        if (!input.hasAttribute("data-supported-currency")) {
            let currency;
            switch (input.value) {
    ...
    // Configurar evento de alteração do provedor
    document.querySelectorAll('input[name="provider"]').forEach((input) => {
        input.addEventListener("change", function () {
            if (this.checked) {
                // Atualizar valor mínimo com base no provedor selecionado
                ...
                // Atualizar seleção de moeda com base no provedor
                const providerValue = this.value;
                const currencySelect = document.getElementById("currency");
                
                // Definir a moeda adequada para o provedor selecionado
