...
"dark" === savedTheme && (document.body.setAttribute("data-theme", "dark"), document.body.setAttribute("data-bs-theme", "dark"), themeToggleIcon.classList.remove("fa-moon"), themeToggleIcon.classList.add("fa-sun")),
    themeToggle.addEventListener("click", () => {
        let e = "light" === (document.body.getAttribute("data-theme") || "light") ? "dark" : "light";
        document.body.setAttribute("data-theme", e),
            document.body.setAttribute("data-bs-theme", e),
            localStorage.setItem("theme", e),
            "dark" === e ? (themeToggleIcon.classList.remove("fa-moon"), themeToggleIcon.classList.add("fa-sun")) : (themeToggleIcon.classList.remove("fa-sun"), themeToggleIcon.classList.add("fa-moon")),
            document.dispatchEvent(new CustomEvent("themeChanged", { detail: { theme: e } })),
            "function" == typeof fixDarkModeStyles && fixDarkModeStyles();
    });
const providerSearch = document.getElementById("provider-search");
function showValidationError(e) {
    let t = document.createElement("div");
    (t.className = "alert alert-danger alert-dismissible fade show"),
        (t.innerHTML = `
  <i class="fas fa-exclamation-circle me-2"></i>${e}
  <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
`),
        (paymentResult.innerHTML = ""),
        paymentResult.appendChild(t),
...
function startLoading() {
    (submitText.textContent = "Processing..."), loadingSpinner.classList.remove("d-none"), (submitBtn.disabled = !0);
}
function stopLoading() {
    (submitText.textContent = "Generate Payment Link"), loadingSpinner.classList.add("d-none"), (submitBtn.disabled = !1);
}
function copyToClipboard(e, t) {
    navigator.clipboard
        .writeText(e)
        .then(() => {
            let e = t.querySelector(".copy-feedback");
            e.classList.add("show-feedback"),
                setTimeout(() => {
                    e.classList.remove("show-feedback");
                }, 2e3);
        })
        .catch((e) => {
            console.error("Error copying: ", e), showToast("Failed to copy to clipboard", "danger");
        });
}
providerSearch.addEventListener("input", () => {
    let e = providerSearch.value.toLowerCase();
    ...
    form.addEventListener("submit", async (e) => {
        if ((e.preventDefault(), !form.checkValidity())) {
            e.stopPropagation(), form.classList.add("was-validated");
            return;
        }
        let t = document.getElementById("wallet_address").value.trim(),
            a = document.getElementById("email_address").value.trim(),
            s = parseFloat(document.getElementById("amount").value),
            r = document.getElementById("currency").value,
            i = document.querySelector('input[name="provider"]:checked').value;
        if (!/^0x[a-fA-F0-9]{40}$/.test(t)) {
            showValidationError("Wallet address must be a valid Ethereum address (0x... format)");
            return;
        }
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(a)) {
        ...
        if (s < minAmounts[i]) {
            showValidationError(`Minimum amount for ${i} is ${minAmounts[i]}`);
            return;
        }
        startLoading(), (paymentResult.innerHTML = ""), paymentResult.classList.remove("show");
        let n = encodeURIComponent(`https://paygate.to/payment-link/invoice.php?payment=${Date.now()}_${Math.floor(9e6 * Math.random()) + 1e6}`);
        try {
            let l = await fetchExternalApi(`https://api.transact.st/control/wallet.php?address=${t}&callback=${n}`);
            if (!l.ok) throw Error(`Server response error: ${l.status}`);
            let o = await l.json();
            if (o && o.address_in) {
                let c = o.address_in;
                o.callback_url;
                let d = `https://payment.transact.st/process-payment.php?address=${c}&amount=${s}&provider=${i}&email=${encodeURIComponent(a)}&currency=${r}`,
                    u = document.createElement("div");
                (u.className = "card mb-4 result-card animate-success"),
                    (u.innerHTML = `
    ...
    `),
                    (paymentResult.innerHTML = ""),
                    paymentResult.appendChild(u),
                    paymentResult.classList.add("show"),
                    document.getElementById("copy-payment-link").addEventListener("click", function () {
                        copyToClipboard(d, this);
                    }),
                    document.getElementById("copy-tracking-number").addEventListener("click", function () {
                        copyToClipboard(c, this);
                    }),
                    document.getElementById("copy-tracking-url").addEventListener("click", function () {
                        copyToClipboard(`https://payment.transact.st/control/track.php?address=${c}`, this);
                    }),
                    document.getElementById("share-payment-link").addEventListener("click", function () {
                        sharePaymentLink(d, r, s);
                    }),
                    fixDarkModeStyles(),
                    setTimeout(() => {
            ...
            } else showValidationError("Invalid payment wallet. Please enter a valid wallet address.");
        } catch (m) {
            console.error("Error:", m), showValidationError(`An error occurred while processing your request: ${m.message}`);
        } finally {
            stopLoading();
        }
    });
...
function showToast(e, t = "info") {
    ...
    (s.className = `toast align-items-center text-white bg-${t} border-0`),
...
`),
        a.appendChild(s);
    new bootstrap.Toast(s, { delay: 3e3 }).show(),
        s.addEventListener("hidden.bs.toast", () => {
            s.remove();
        });
}
function sharePaymentLink(e, t = "USD", a = 0) {
    navigator.share
        ? navigator
              .share({ title: "Transact Payment Link", text: `Use this link to make a payment of ${a} ${t}:`, url: e })
              .then(() => showToast("Link shared successfully", "success"))
              .catch((e) => console.log("Error sharing:", e))
        : (copyToClipboard(e, document.getElementById("copy-payment-link")), showToast("Link copied to clipboard", "success"));
}
function fixDarkModeStyles() {
    if ("dark" === document.body.getAttribute("data-theme") || "dark" === document.body.getAttribute("data-bs-theme")) {
        document.querySelectorAll(".copy-link-input, #payment-link").forEach((e) => {
            (e.style.backgroundColor = "#1e293b"), (e.style.color = "#f8fafc"), (e.style.borderColor = "#334155");
        });
        document.querySelectorAll(".copy-link-container").forEach((e) => {
            (e.style.backgroundColor = "#1e293b"), (e.style.borderColor = "#334155");
        });
        document.querySelectorAll(".card-header .btn-light, .card-header .btn-sm").forEach((e) => {
            (e.style.backgroundColor = "#334155"), (e.style.color = "#f8fafc"), (e.style.borderColor = "#475569");
        });
    }
}
function filterProvidersByCurrency(e) {
    ...
    t.forEach((t) => {
        ...
        "ALL" === r || r === e ? ((t.style.display = "block"), a || ((s.checked = !0), (a = !0))) : ((t.style.display = "none"), (s.checked = !1));
    });
}
async function fetchExternalApi(e, t = {}) {
    try {
        let a = { ...t, credentials: "omit" },
            s = await fetch(e, a);
        if (!s.ok) throw Error(`API request failed with status: ${s.status}`);
        return s;
    } catch (r) {
        throw (console.error("External API fetch error:", r), r);
    }
}
providerInputs.forEach((e) => {
    ...
    document.querySelectorAll('input[name="provider"]').forEach((e) => {
        e.addEventListener("change", function () {
            ...
            let a = minAmounts[e] || 0,
                s = document.getElementById("amount");
            s.setAttribute("min", a),
                s.setAttribute("placeholder", `Min: ${a}`),
    ...
    document.getElementById("amount").addEventListener("change", function () {
        let e = parseFloat(this.value),
            t = document.querySelector('input[name="provider"]:checked').value,
            a = minAmounts[t] || 0;
        e < a ? (this.setCustomValidity(`Minimum amount for ${t} is ${a}`), showToast(`Minimum amount for ${t} is ${a}`, "warning")) : this.setCustomValidity("");
    }),
