document.addEventListener("DOMContentLoaded", function () {
  ...
  function B() {
      (e.style.display = "block"), (t.style.display = "none"), (a.style.display = "none"), (s.style.display = "none"), (r.style.display = "none"), (l.style.display = "none");
  }
  function I() {
      (e.style.display = "none"), (t.style.display = "block"), (s.style.display = "block"), (r.style.display = "block"), (l.style.display = "block");
  }
  function q() {
      (o.style.display = "block"), (c.style.display = "none"), i.classList.add("active"), d.classList.remove("active");
  }
  function A(e) {
      if (e) {
  ...
  function T() {
      document.querySelectorAll(".table-responsive").forEach((e) => {
          e.classList.add("table-has-scroll"), e.scrollWidth > e.clientWidth ? e.classList.add("can-scroll") : e.classList.remove("can-scroll"), e.removeEventListener("scroll", j), e.addEventListener("scroll", j);
      });
  }
  function j() {
      let e = this.scrollLeft + this.clientWidth >= this.scrollWidth - 10;
      e ? this.classList.remove("can-scroll") : this.scrollWidth > this.clientWidth && this.classList.add("can-scroll");
  }
  async function C() {
      try {
          var e;
          let t = await fetch("/api/admin/invitation-requests", { headers: { Authorization: `Bearer ${L}` } });
          if (401 === t.status) {
              ...
              0 === n.length
                  ...
                  : ((g.style.display = "none"),
                    ...
                    e.forEach((e) => {
                        ...
                        "approved" === e.status ? n.classList.add("table-success") : "rejected" === e.status && n.classList.add("table-danger"),
                            (n.innerHTML = `
        <td>${e.id}</td>
        <td>${e.email}</td>
      ...
      `),
                    ...
                    document.querySelectorAll(".approve-btn").forEach((e) => {
                        e.addEventListener("click", function () {
                            (document.getElementById("request-id").value = this.dataset.id), p.show();
                        });
                    }),
                    document.querySelectorAll(".reject-btn").forEach((e) => {
                        e.addEventListener("click", async function () {
                            if (confirm("Are you sure you want to reject this request?")) {
                                ...
                                try {
                                    let t = await fetch(`/api/admin/invitation-requests/${e}/reject`, { method: "POST", headers: { Authorization: `Bearer ${L}` } });
                                    if (t.ok) C();
                                    else {
                                        let n = await t.json();
          ...
          console.error("Error loading invitation requests:", a), alert("Failed to load invitation requests");
      }
  }
  async function S() {
      try {
          let e = await fetch("/api/admin/invitation-codes", { headers: { Authorization: `Bearer ${L}` } });
          if (401 === e.status) {
              localStorage.removeItem("adminToken"), B();
              return;
          }
              ...
              0 === t.length
                  ...
                  : (a && (a.style.display = "none"),
                    (function e(t) {
                        ...
                        if ((a || console.error("Element #no-codes not found in renderCodesTable!"), !t || 0 === t.length)) {
                            (n.innerHTML = ""), a && (a.style.display = "block");
                            return;
                        }
                        a && (a.style.display = "none"),
                            ...
                            t.forEach((e, t) => {
                                ...
                                let s = document.createElement("tr"),
                                    ...
                                    r = e.last_used_at ? new Date(e.last_used_at).toLocaleDateString() : "Never";
                                e.is_active ? new Date(e.expires_at) < new Date() && s.classList.add("table-warning") : s.classList.add("table-danger");
                                let l = e.is_active ? (new Date(e.expires_at) < new Date() ? "Expired" : "Active") : "Revoked";
                                (s.innerHTML = `
        <td>${e.id || "-"}</td>
        ...
        <td class="status-column"><span class="badge ${(function e(t) {
            if (!t) return console.warn("Status is undefined or null"), "bg-secondary";
            switch (t) {
                case "Active":
                    return "bg-success";
                case "Expired":
      ...
      `),
                                ...
                                t.forEach((e, t) => {
                                    e.addEventListener("click", async function () {
                                        if (confirm("Are you sure you want to revoke this invitation code?")) {
                                            let e = this.dataset.id;
                                            try {
                                                let t = await fetch(`/api/admin/invitation-codes/${e}/revoke`, { method: "POST", headers: { Authorization: `Bearer ${L}` } });
                                                if (t.ok) S();
                                                else {
                                                    let n = await t.json();
          ...
          console.error("Error loading invitation codes:", s), alert("Failed to load invitation codes");
      }
  }
  v.setAttribute("data-bs-theme", w),
      ...
      b.addEventListener("click", function () {
          let e = v.getAttribute("data-bs-theme"),
              t = "dark" === e ? "light" : "dark";
          v.setAttribute("data-bs-theme", t), localStorage.setItem("theme", t), this.classList.toggle("light");
      }),
      L ? (I(), q(), C()) : B(),
      n.addEventListener("submit", async function (e) {
          ...
          try {
              let s = await fetch("/api/admin/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: t, password: n }) }),
                  i = await s.json();
              s.ok ? ((L = i.token), localStorage.setItem("adminToken", L), I(), q(), C()) : ((a.textContent = i.error || "Invalid credentials"), (a.style.display = "block"));
          } catch (d) {
              (a.textContent = "An error occurred. Please try again."), (a.style.display = "block");
          }
      }),
      ...
      d.addEventListener("click", function (e) {
          e.preventDefault(), (o.style.display = "none"), (c.style.display = "block"), d.classList.add("active"), i.classList.remove("active"), S();
      }),
      ...
      h.addEventListener("click", async function () {
          ...
          try {
              let n = await fetch(`/api/admin/invitation-requests/${e}/approve`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${L}` }, body: JSON.stringify({ type: t }) });
              if (n.ok) p.hide(), C(), S();
              else {
                  let a = await n.json();
